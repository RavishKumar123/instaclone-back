...
module.exports.retrieveUser = async (req, res, next) => {
  ...
  try {
    ...
    var posts = { data: [], postCount: totalBookmarks };
    var count = 0;
          ...
          Post.aggregate([
            {
              $facet: {
                data: [
                  { $match: { _id: ObjectId(bookmark.post) } },
                  { $sort: { date: -1 } },
                  { $limit: 12 },
                  {
                    $lookup: {
            ...
            posts.data.push(res1[0].data[0]);
            count++;
            if (totalBookmarks === count) {
              console.log(
                "yes because cound : " + count + " and total" + totalBookmarks
              );
              return res.send({
                user,
...
module.exports.retriveBrand = async (req, res, next) => {
  ...
  try {
    const brand = await Brand.findOne(
      { _id: brandId },
      "name description domain affilate_link avatar"
    );
    if (!brand) {
      return res
        .status(404)
...
module.exports.followBrand = async (req, res, next) => {
  const { brandId } = req.params;
  const user = res.locals.user;

  try {
    const brandToFollow = await Brand.findById(brandId);
    if (!brandToFollow) {
      return res
        .status(400)
        .send({ error: "Could not find a brand with that id." });
    }

    const followerUpdate = await BrandFollowers.updateOne(
      { brand: brandId, "followers.user": { $ne: user._id } },
      { $push: { followers: { user: user._id } } }
    );

    const followingUpdate = await Following.updateOne(
      { user: user._id, "followingBrand.brand": { $ne: brandId } },
      { $push: { followingBrand: { brand: brandId } } }
    );
    ...
    if (!followerUpdate.nModified || !followingUpdate.nModified) {
      if (!followerUpdate.ok || !followingUpdate.ok) {
        return res
          .status(500)
          .send({ error: "Could not follow brand please try again later." });
      }
      // Nothing was modified in the above query meaning that the user is already following
      // Unfollow instead
      const followerUnfollowUpdate = await BrandFollowers.updateOne(
        {
          brand: brandId,
        },
        { $pull: { followers: { user: user._id } } }
      ...
      const followingUnfollowUpdate = await Following.updateOne(
        { user: user._id },
        { $pull: { followingBrand: { brand: brandId } } }
      );
      if (!followerUnfollowUpdate.ok || !followingUnfollowUpdate.ok) {
        return res
          .status(500)
...
module.exports.searchUsers = async (req, res, next) => {
  ...
  try {
    ...
    return res.send(users.concat(brands));
  } catch (err) {
    next(err);
  }
};
...
module.exports.retrieveSuggestedUsers = async (req, res, next) => {
  ...
  const user = res.locals.user;
  try {
    const users = await Brand.aggregate([
      {
        $match: {},
      },
      {
